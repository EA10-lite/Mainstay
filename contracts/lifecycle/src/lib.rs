...
impl Lifecycle {
    ...
    pub fn get_collateral_score(env: Env, asset_id: u64) -> u32 {
        ...
        Self::get_collateral_score_unchecked(&env, asset_id)
    }

    /// Get the collateral score for an asset without verifying asset existence.
    /// 
    /// This is an internal optimization to avoid cross-contract calls when the caller
    /// already knows the asset exists. Use the public `get_collateral_score` for safety.
    fn get_collateral_score_unchecked(env: &Env, asset_id: u64) -> u32 {
        env.storage()
            .persistent()
            .get(&score_key(asset_id))
            .unwrap_or(0)
...
mod tests {
    ...
    #[test]
    fn test_is_collateral_eligible_matches_score_check() {
        let env = Env::default();
        env.mock_all_auths();

        let (client, asset_registry_client, engineer_registry_client, admin) = setup(&env, 50);
        let engineer = Address::generate(&env);
        let asset_id = register_test_asset(&asset_registry_client, &admin);
        register_test_engineer(&engineer_registry_client, &admin, &engineer);

        // Submit maintenance to get a score above threshold
        client.submit_maintenance(&asset_id, &String::from_str(&env, "routine"), &String::from_str(&env, "test"), &engineer);
        
        let score = client.get_collateral_score(&asset_id);
        let is_eligible = client.is_collateral_eligible(&asset_id);
        
        // Verify eligibility matches score >= threshold comparison
        assert_eq!(is_eligible, score >= 50);
    }
